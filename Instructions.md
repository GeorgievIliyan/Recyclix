...
2. **Настройка на Supabase Dashboard**  
    ...
    За да създадете автоматично нужните таблици и защитни политики (RLS).
        ...
        > Забележка: В директорията на проекта се намира файл на име `schema.sql` - той е пълен и изчерпателен файл за PostgreSQL клиент да създаде 1:1 копие на схемата на базата данни.
...
3. **Употреба:**  
    **Достъпни URL пътища на приложението**:
    ...
    /auth/register
    /auth/reset-password
    # интерфейс на smart контейнери (използвайте парола "admin")
    /bin/scan 
    ```
