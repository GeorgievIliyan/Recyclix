...
const Btn = ({
  ...
  <button
    onClick={onClick}
    className={`flex-1 px-6 py-3 rounded-xl text-sm font-semibold transition-all duration-200 ${variant === "primary"
...
const Loading = () => {
  ...
  return (
    <Card>
      ...
      <Title>{t("claim.loading.title")}</Title>
    </Card>
  );
};
...
const Unauthenticated = ({ handleLogin, router }: any) => {
  ...
  return (
    <Card>
      ...
      <Title>{t("claim.unauthenticated.title")}</Title>
      <Subtitle>{t("claim.unauthenticated.subtitle")}</Subtitle>
      <BtnRow>
        <Btn variant="primary" onClick={handleLogin}>{t("claim.unauthenticated.login")}</Btn>
        <Btn onClick={() => router.back()}>{t("claim.buttons.back")}</Btn>
      </BtnRow>
    </Card>
  );
};
...
const Claimed = ({ handleDashboard, router }: any) => {
  ...
  return (
    <Card>
      ...
      <Title>{t("claim.claimed.title")}</Title>
      <Subtitle>{t("claim.claimed.subtitle")}</Subtitle>
      <BtnRow>
        <Btn onClick={handleDashboard}>{t("claim.buttons.dashboard")}</Btn>
        <Btn variant="primary" onClick={() => router.back()}>{t("claim.buttons.back")}</Btn>
      </BtnRow>
    </Card>
  );
};
...
const Expired = ({ handleDashboard, router }: any) => {
  ...
  return (
    <Card>
      ...
      <Title>{t("claim.expired.title")}</Title>
      <Subtitle>{t("claim.expired.subtitle")}</Subtitle>
      <BtnRow>
        <Btn onClick={handleDashboard}>{t("claim.buttons.dashboard")}</Btn>
        <Btn onClick={() => router.back()}>{t("claim.buttons.back")}</Btn>
      </BtnRow>
...
const Invalid = ({ handleDashboard, router }: any) => {
  ...
  return (
    <Card>
      ...
      <Title>{t("claim.invalid.title")}</Title>
      <Subtitle>{t("claim.invalid.subtitle")}</Subtitle>
      <BtnRow>
        <Btn onClick={handleDashboard}>{t("claim.buttons.dashboard")}</Btn>
        <Btn onClick={() => router.back()}>{t("claim.buttons.back")}</Btn>
      </BtnRow>
...
const Success = ({
  ...
  return (
    <Card>
      ...
      <Title>{t("claim.success.title")}</Title>

      {/* Спечелени точки */}
      <div className="w-full rounded-xl bg-green-500/10 border border-green-500/20 px-6 py-4 mb-3 mt-3">
        <p className="text-xs text-muted-foreground uppercase tracking-widest mb-1">{t("claim.success.pointsEarned")}</p>
      ...
      <div className="w-full rounded-xl bg-muted/50 border border-border px-6 py-4 mb-6">
        <p className="text-xs text-muted-foreground uppercase tracking-widest mb-1 flex items-center justify-center gap-1.5">
          ...
          {t("claim.success.totalXp")}
        </p>
        <div className="text-4xl font-semibold text-foreground">{newTotal.toLocaleString(i18n.language)}</div>
      </div>

      <BtnRow>
        <Btn variant="primary" onClick={handleDashboard}>{t("claim.buttons.dashboard")}</Btn>
        <Btn onClick={() => router.back()}>{t("claim.buttons.back")}</Btn>
      </BtnRow>
    </Card>
  );
...
// Основна логика за вземане на точки
function ClaimContent() {
  ...
  if (!mounted) return <CenteredLoader />;
