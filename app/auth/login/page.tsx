...
export default function LoginPage() {
  ...
  const handleEmailLogin = async () => {
    ...
    if (!cleanEmail || !cleanPassword) {
      setIsSuccess(false);
      setMessage(t("auth.login.messages.requiredFields"));
      return;
    }
    if (!/^\S+@\S+\.\S+$/.test(cleanEmail)) {
      setIsSuccess(false);
      setMessage(t("auth.login.messages.invalidEmail"));
      return;
    }
    ...
    try {
      ...
      if (error) {
        ...
        if (msg === "invalid login credentials") {
          setMessage(t("auth.login.messages.invalidCredentials"));
        } else if (msg === "invalid input") {
          setMessage(t("auth.login.messages.invalidInput"));
        } else {
          setMessage(t("auth.login.messages.unexpectedError"));
        }
      } else {
  ...
  const handleGoogleLogin = async () => {
    try {
      ...
      if (error) {
        setIsSuccess(false);
        setMessage(t("auth.login.messages.googleFail"));
      }
    } catch {
      setIsSuccess(false);
      setMessage(t("auth.login.messages.googleFail"));
